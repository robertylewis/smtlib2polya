...
from parser2.smtparser import SMTParser, SMTParseException
...
g_cmd_kinds   = \
    [ KIND_ASSERT,   KIND_CHECKSAT, KIND_DECLFUN,   KIND_DEFFUN, 
      ...
      KIND_SETLOGIC, KIND_SETINFO,  KIND_SETOPT,    KIND_SIMPLIFY ]
...
class SMTSortNode (SMTNode):
    ...
    def __init__ (self, name, nparams = 0, kind = KIND_SORT):
        super(SMTSortNode, self).__init__(kind)
        self.name = name
...
class SMTArraySortNode (SMTSortNode):
    ...
    def __init__ (self, index_sort = None, elem_sort = None):
        super(SMTArraySortNode, self).__init__(
...
class SMTConstNode (SMTNode):
    ...
    def __init__ (self, kind, sort, value = 0):
        ...
        super(SMTConstNode, self).__init__(kind, sort)
        self.value = value
...
class SMTFunNode (SMTNode):
    ...
    def __init__ (self, name, sort, sorts = [], indices = [], children = []):
        ...
        super(SMTFunNode, self).__init__(KIND_FUN, sort, children)
...
class SMTAnFunNode (SMTNode):
    ...
    def __init__ (self, fun, sort):
        super(SMTAnFunNode, self).__init__(KIND_ANNFUN, sort)
        self.fun = fun

    def __sizeof (self):
        return super(SMTAnFunNode, self).__sizeof__() + self.fun.__sizeof__()
...
class SMTFunAppNode (SMTNode):        
    ...
    def __init__ (self, fun, kind, sort, children):
        ...
        super(SMTFunAppNode, self).__init__(kind, sort, children)
        self.fun = fun
...
class SMTSortedQVarNode (SMTNode):
    ...
    def __init__ (self, var):
        assert (isinstance (var, SMTFunNode))
        assert (var.sort)
        super(SMTSortedQVarNode, self).__init__(KIND_SVAR, var.sort)
        self.var = var
...
class SMTForallExistsNode (SMTNode):
    ...
    def __init__ (self, svars, kind, children):
        ...
        assert (len(children) == 1)
        super(SMTForallExistsNode, self).__init__(kind, children[0].sort, children)
        self.svars = svars
...
class SMTLetNode (SMTNode):
    ...
    def __init__ (self, children):
        super(SMTLetNode, self).__init__(KIND_LET, children[-1].sort, children)
...
class SMTAnnNode (SMTNode):
    ...
    def __init__ (self, attributes, sort, children):
        assert (len(children) == 1)
        super(SMTAnnNode, self).__init__(KIND_ANNOTN, sort, children)
...
class SMTNamedAnnNode (SMTAnnNode):
    ...
    def __init__ (self, attributes, sort, children, name):
        super(SMTNamedAnnNode, self).__init__(attributes, sort, children)
...
class DDSMTParser (SMTParser):
    ...
    def __init__ (self):
        super(DDSMTParser, self).__init__()
        self.smtformula = SMTFormula()
        self.__set_parse_actions()

    def parse (self, infile):
        try:
            super(DDSMTParser, self).parse(infile)
